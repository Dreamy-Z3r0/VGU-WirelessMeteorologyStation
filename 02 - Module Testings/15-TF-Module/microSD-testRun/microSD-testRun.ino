...
//  microSD.add_entry(ChangeLog, "Boot.");
  ...
  cardData = SD.open("CHANGELOG.txt", FILE_WRITE);
  if (cardData) {
    cardData.println("Boot.");
    cardData.close();
  } else {
    Serial.println("Error accessing microSD card: first change log");
  }
...
//  microSD.add_entry(SystemConfig, String("Date and time changed: \n" + SystemConfigChange));
  ...
  cardData = SD.open("CHANGELOG.txt", FILE_WRITE);
  if (cardData) {
    cardData.print("Date and time changed: \n");
    cardData.print(SystemConfigChange);
    cardData.close();
  } else {
    Serial.println("Error acccessing card: end of setup()");
  }
}
...
void LoggingPseudoroutine(String* Pseudochanges) {
  ...
  // Update daily log
  *Pseudochanges += (DateData + "," + TimeData + ",");
  *Pseudochanges += (String(dataIndex) + ",");
  
  if (DAILY_ALARM == RTC_Alarm_Flag) {
    *Pseudochanges += "Daily alarm,";
  } else if (MINUTE_ALARM == RTC_Alarm_Flag) {
    if (CUSTOM_MINUTE_ALARM > 1) {
      *Pseudochanges += "Custom period alarm,";
    } else {
      *Pseudochanges += "Minute alarm,";
    }
  } else if (DUAL_ALARM == RTC_Alarm_Flag) {
    *Pseudochanges += "Daily alarm+,";
  } else {
    *Pseudochanges += "NONE (first run),";
  }
  
  *Pseudochanges += (String(randomTemperatureGround) + "," + String(randomTemperatureAir) + ",");
  *Pseudochanges += (String(randomHumidity) + "," + String(randomPressure) + "\n");

//  microSD.add_entry(DailyLog, *Pseudochanges);
  if (!SD.exists("DAILY_LOG")) {
    SD.mkdir("DAILY_LOG");
  }

  String DAILY_LOG_FILENAME = "DAILY_LOG/" + DateData + ".csv";
  cardData = SD.open("DAILY_LOG_FILENAME", FILE_WRITE);
  if (cardData) {
    cardData.print(*Pseudochanges);
    cardData.close();
  } else {
