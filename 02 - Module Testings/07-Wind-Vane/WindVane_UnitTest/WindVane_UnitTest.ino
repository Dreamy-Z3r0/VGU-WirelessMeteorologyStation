...
//  SPI.setSSEL(PA4);
  ...
  String FirstLine = "Direction (read),Raw ADC,V_in,R_read";
...
void loop() {
  if (Serial.available()) {
    ...
    String manual_reading;
    Serial.println("Enter manual voltage reading.");
    while (!next_input) {
      if (Serial.available()) {
        manual_reading = Serial.readStringUntil('\n');
        next_input = true;
      }
    }
    ...
    for (uint8_t loop_index = 0; loop_index < 20; loop_index += 1) {
      output_ready = false;
      MX_ADC1_Init(true);
      HAL_ADC_Start_DMA(&hadc1, &raw, 1);
      ...
      V_in = (uint16_t)raw * (V_ref / ADC_MAX_VALUE);
