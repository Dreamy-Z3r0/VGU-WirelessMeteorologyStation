                ...
                case DailyLog: { // "DAILY_LOG/YYYY-MM-DD.csv"
                    ...
                    if (!SD.exists(FileToWrite)) {
                        ...
                        if (cardData) {     // Add first row of column identifiers
                            String firstRow_Meteorological = "Time -> Date,Rainfall amount -> Time,Wind speed -> dataIndex,Wind direction -> Alarm type,";
                            String firstRow_Temperature = "Ambient temperature (ground) -> random,Ambient temprature (1m) -> random,";
                            String firstRow_Unverifiable = "Relative humidity -> random,Barometric pressure -> random";
