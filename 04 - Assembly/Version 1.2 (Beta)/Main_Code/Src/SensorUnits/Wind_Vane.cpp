...
// Initiate the ADC
void WindVane_Control::init(TIM_TypeDef* StandbyTimer_Instance) {
  StandbyTimer = new HardwareTimer(StandbyTimer_Instance);

  StandbyTimer->setMode(1, TIMER_DISABLED);    // Use channel 1 of StandbyTimer_Instance in output compare mode, no output
  StandbyTimer->setOverflow(1000, MICROSEC_FORMAT);   // Timer overflows every 1ms
  StandbyTimer->attachInterrupt(1, std::bind(STANDBY_TIM_Ovf_callback, this));  // ISR run whenever timer overflows for channel 1
...
void WindVane_Control::Timer_Callback(void) {
  standby_counter += 1;

  if (standby_period <= standby_counter) {
    StandbyTimer->pause();
    set_readFlag();
...
void STANDBY_TIM_Ovf_callback(WindVane_Control *WindVane_Instance) {
  WindVane_Instance->Timer_Callback();
}
